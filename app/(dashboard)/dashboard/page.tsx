...
interface Tour {
  ...
  endDate: string;
  coverImage?: string;
  coverImages?: string[]; // ADD THIS LINE
    ...
    try {
      ...
      if (res.ok && Array.isArray(data)) {
        ...
        const totalExpenses = data.reduce(
          (sum: number, tour: Tour) => sum + (tour.expenses?.length || 0),
          0,
        );
        const totalMembers = data.reduce(
          (sum: number, tour: Tour) => sum + (tour.members?.length || 0),
          0,
        );
        const upcomingTrips = data.filter(
          (tour: Tour) => new Date(tour.startDate) > new Date(),
        ).length;
    ...
    <div className="container mx-auto px-6">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-[#e8f0eb] mt-1">
                {stats.totalTours}
              </p>
            </div>
      ...
      {tours.length === 0 ? (
        <div className="text-center py-16 bg-[#121816]/80 backdrop-blur-lg border border-[#2a322e] rounded-2xl shadow-xl">
          ...
          <h3 className="text-xl font-semibold text-[#e8f0eb] mb-2">
            No tours yet
          </h3>
          <p className="text-[#a0b0a8] mb-6">
            Create a tour or join one with an invitation code!
          </p>
      ...
      {showJoinModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <p className="text-[#a0b0a8] mb-4">
              Enter the invitation code shared by your friend:
            </p>
