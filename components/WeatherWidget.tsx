...
import { FiCloud, FiCloudRain, FiSun, FiWind, FiDroplet, FiThermometer } from 'react-icons/fi';
...
export default function WeatherWidget({ destination }: WeatherWidgetProps) {
  ...
  const getWeatherIcon = (iconCode: string) => {
    if (iconCode?.includes('01')) return <FiSun className="w-8 h-8 text-yellow-400" />;
    if (iconCode?.includes('02')) return <FiCloud className="w-8 h-8 text-[#a0b0a8]" />;
    if (iconCode?.includes('09') || iconCode?.includes('10')) return <FiCloudRain className="w-8 h-8 text-blue-400" />;
  ...
  if (error || !weather) {
    return (
      <div className="bg-[#121816]/80 backdrop-blur-lg border border-[#2a322e] rounded-2xl p-6 shadow-xl">
        <p className="text-[#6b7a72] text-center">Weather information unavailable</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold text-[#e8f0eb] mb-4">Weather in {weather.current.city}</h3>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-[#a0b0a8] capitalize">{weather.current.description}</p>
          </div>
        </div>
      ...
      {weather.forecast.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-[#a0b0a8] mb-3">Upcoming Forecast</h4>
          <div className="grid grid-cols-4 gap-2">
            {weather.forecast.map((forecast: any, index: number) => (
              <div key={index} className="bg-[#1a211e] rounded-lg p-3 text-center">
                <p className="text-xs text-[#6b7a72]">
                  {new Date(forecast.time).toLocaleTimeString([], { hour: '2-digit' })}
                </p>
